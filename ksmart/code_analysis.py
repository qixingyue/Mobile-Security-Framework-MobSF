...
import os
import settings
from MalwareAnalyzer import MalwareCheck

from util import PrintException,escape
...
def code_analysis(app_dir, md5, perms, typ):
    ...
    try:
        ...
        for dir_name, sub_dir, files in os.walk(java_src):
            for jfile in files:
                jfile_path = os.path.join(dir_name, jfile)
                if "+" in jfile:
        ...
        PrintException("[ERROR] Performing Code Analysis",e)
